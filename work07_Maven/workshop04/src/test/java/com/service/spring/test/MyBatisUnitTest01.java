...
public class MyBatisUnitTest01 {
	public static void main(String [] args) throws Exception {
		...
		//쿼리문 실행
		System.out.print("0. addProduct : ");
		MyProduct myProduct = new MyProduct("NoteBook", "LG", 5000);
		System.out.println(session.insert("ns.sql.MyProductMapper.addProduct", myProduct));
		session.commit();
		System.out.println("\n");
		
		System.out.println("1. findProductByName");
		List<MyProduct> productsByName = session.selectList("ns.sql.MyProductMapper.findProductByName", "TV");
		for(MyProduct i: productsByName) System.out.println(i);
		session.commit();
		System.out.println("\n");
		
		System.out.println("2. findProductByMaker");
		List<MyProduct> productsByMaker= session.selectList("ns.sql.MyProductMapper.findProductByMaker","LG");
		for(MyProduct i: productsByMaker) System.out.println(i);
		session.commit();
		System.out.println("\n");
		
		System.out.println("3. findProducts");
		List<MyProduct> products= session.selectList("ns.sql.MyProductMapper.findProducts");
		session.commit();
