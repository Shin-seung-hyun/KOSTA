...
/* 
  ...
  이를 이름, 급여, 회비(소수점 세번째자리에서 반올림)를 출력하라.
*/
...
select empno, ename, sal, ceiling(com  * 1.50) as increase_sal
from emp
where (com != 0) and (com is not null);
...
select ename, deptno, sal, (sal *12 + ifnull(comm,0)) + sal * 1.70 as annual_sal
from emp
where deptno = 30;

--  6) emp Table에서 이름, 급여, 커미션 금액, 총액(sal + comm)을  구하여 총액이 많은 순서로 출력하라. 
--  단, 커미션이 NULL인 사람은 제외한다.(커미션금액: sal*comm/100)
select ename, sal, (sal + comm) '커미션 금액'
from emp
where (sal*comm/100) is not null
order by 3 desc;
