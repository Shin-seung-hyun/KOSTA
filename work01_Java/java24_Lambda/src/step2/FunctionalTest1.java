...
/*
	자주 사용되는 다양한 함수형 인터페이스 제공
	 ...
	 Supplier<T>   T get()            공급자   매개변수 X, 반환값 O (= Get 가져오는 기능)
	 Consumer<T>   void accept(T t)   소비자   매개변수 O, 반환값 X (= Set 주입/수정)
	 Function<T,R>  R apply(T t)      함수    매개변수 O, 반환값 O (= 함수적 기능)
	 Predicate<T>   boolean test(T t) 조건식   매개변수 O, 반환값 O boolean (= 조건에 따라 T/F 반환)
	----------------------------------------------------------------------------------------------------
...
public class FunctionalTest1 {
	...
	//클래스 안 만들기 위해.. 매개변수로 Hasing 하게 만들자
	...
	public static void printEvenNum(Predicate<Integer> p, 
		...
		System.out.print("[");
		for(Integer i : list) {
			if(p.test(i)) //만약에 i 값이 짝수라면 해당하는 값을 출력
				c.accept(i);			 
		}
	...
	public static void main(String[] args) {
		...
		System.out.println("\n\n--------- 4. Function ---------");
		Function<Integer, Integer> f = i-> i/10 * 10;	//i 값에 일의 자리를 없애는 함수
