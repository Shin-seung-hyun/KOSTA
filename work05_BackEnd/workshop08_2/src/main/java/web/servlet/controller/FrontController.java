...
public class FrontController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("login")) {
			...
			path = detailView(request, response);
		}   
		
		request.getRequestDispatcher(path).forward(request, response);
	}

	private String detailView(HttpServletRequest request, HttpServletResponse response) {
	...
	private String registerBook(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			request.setAttribute("book", book);
			request.setAttribute("msg", "책 정보가 정상적으로 저장됐습니다!");
			...
			//사용자가 보는 메시지
			request.setAttribute("msg", "책 등록중 오류 발생! 정보 저장 실패");
			
			return "../error/Error.jsp";  // error 페이지로 이동
		}		
	}
