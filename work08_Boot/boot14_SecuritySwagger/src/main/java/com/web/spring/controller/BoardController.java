...
public class BoardController {
	...
	private final BoardService boardService;
		...
		* */
	    ...
	    /**
		 * 게시물 등록 
		 * 인증 받지 않으면 403 Forbidden 발생함
		 * 인증을 받으려면, login -> bare 헤더에 넣어주고 post를 해야함
		 * */
//		 @PostMapping("/boards/board") public ResponseEntity<?> save(HttpServletRequest request, @RequestBody BoardReq board){
...
//		 }
		...
		public ResponseEntity<?> save(@RequestBody BoardReq board) {
			...
			if(name!=null && !name.equals("anonymousUser")) { //인증을 안 받았거나 토큰이 없다면
				...
				if(board.getMemberNo() == (m.getMemberNo())) {
					return new ResponseEntity<>(boardService.addBoard(board),HttpStatus.CREATED);//201 
				}
